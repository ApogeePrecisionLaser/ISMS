 ...
 * @author Vikrant
 */
public class DeviceRegController extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (JQstring != null) {
				...
				if (JQstring.equals("searchPersonId")) {
					String q2 = request.getParameter("str2");
					String q3 = request.getParameter("str3");
					list = sm.getPersonId(q, q2, q3, login_office_id);
				}
				if (JQstring.equals("searchRfId")) {
					String q2 = request.getParameter("str2");
					list = sm.getDeviceRfId(q, q2);
				}
				if (JQstring.equals("searchDeviceId")) {
					list = sm.getDeviceId(q);
				}
				if (JQstring.equals("searchDesignation")) {
					list = sm.getDesignation(q);
				}
				if (JQstring.equals("searchClass")) {
					list = sm.getClass(q, login_office_id);
				}
				if (JQstring.equals("searchSection")) {
					String q2 = request.getParameter("str2");
					list = sm.getSection(q, q2, login_office_id);
				}
				if (JQstring.equals("searchRollNo")) {
					String q2 = request.getParameter("str2");
					String q3 = request.getParameter("str3");
					String q4 = request.getParameter("str4");
					list = sm.getRollNo(q, q2, q3,q4,login_office_id);
				}
				
				if(JQstring.equals("getAllDetails")) {
					list = sm.getAllDetails(q,login_office_id);
		...
		if (task.equals("Save")) {
			String designation = request.getParameter("designation_id");
			String class_name = request.getParameter("class_id");
			String section = request.getParameter("section_id");
			String rollNo = request.getParameter("rollno_id");
			String device_name = request.getParameter("device_id");
			String name = request.getParameter("person_id");
			String rfid = request.getParameter("rf_id");
			// System.out.println("name ----"+name+" rfid ---"+rfid);
			sm.saveData(name, rfid, device_name);
			
			session.setAttribute("device_name", device_name);
		...
		}else if (task.equals("generateMapReport"))//start from here
        {
            ...
            listAll = sm.showRegData(login_office, login_office_id);
            //System.out.println("get real path -"+ctx.getRealPath("JSP"));
            jrxmlFilePath = ctx.getRealPath("/report/Blank_A4.jrxml");
            byte[] reportInbytes = sm.generateMapReport(jrxmlFilePath, listAll);
		...
		//session.setAttribute("device_name", device_name);
		//String device_name_s=session.getAttribute("device_name").toString().toUpperCase();
		//System.out.println("device name ss-- "+device_name_s);
		...
		try {
			...
			List<VisitAttendModel> list = VisitAttendModel.showRegData(login_office, login_office_id);
			...
			request.setAttribute("device_name", device_name_s);
						

			request.setAttribute("message", sm.getMessage());
			request.setAttribute("color", sm.getMsgBgColor());
			sm.closeConnection();
			request.getRequestDispatcher("rfid_allocation").forward(request, response);
